...
def update(dt):
	...
	entities.load_asteroids(game_objects, 300)
	...
	if not player_ship.dead:
		...
		for obj in game_objects[1::2]:
			...
			if abs(player_ship.x - obj.x) < 5 and nearly_distance not in player_ship.score_table and abs(player_ship.y - obj.y) < res.ASTEROIDS_DISTANCE:
				player_ship.score_table.append(nearly_distance)

	score_label.text = 'Score: ' + str(len(player_ship.score_table))
